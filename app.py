...
DEFAULT_LAT, DEFAULT_LON = 40.7128, -74.0060  # New York
...
def kv_load(key, default):
      with Session() as s:
                row = s.get(KV, key)
                if row is None:
                              kv_save(key, default); return default
...
def kv_save(key, data):
      with Session() as s:
                ...
                if row is None: s.add(KV(key=key, value=payload))
...
def migrate_legacy_files():
      ...
      for key, path in mapping:
                if not path.exists(): continue
                          with Session() as s:
                                        if s.get(KV, key) is not None: continue
                                                  try:
                                                                kv_save(key, json.loads(path.read_text(encoding="utf-8")))
except Exception: pass
    legacy_notes = DATA_DIR / "notes.txt"
    with Session() as s:
              notes_exists = s.get(KV, "notes") is not None
          if legacy_notes.exists() and not notes_exists:
                    try: text = legacy_notes.read_text(encoding="utf-8")
except Exception: text = ""
        notes = []
        for i, line in enumerate([ln.strip() for ln in text.splitlines() if ln.strip()]):
                      notes.append({"id": uuid.uuid4().hex[:8], "text": line,
                                                              "color": NOTE_COLORS[i % len(NOTE_COLORS)], "done": False})
                  kv_save("notes", notes)
...
@app.route("/healthz")
def healthz(): return jsonify({"ok": True, "t": datetime.utcnow().isoformat()})

# Today's game
@app.route("/api/todays_game")
def todays_game():
      games = kv_load("games", DEFAULT_GAMES)
      if not games: return jsonify({"game": None})
...
def get_notes():
    ...
    for i, n in enumerate(notes):
              if not n.get("id"): n["id"] = uuid.uuid4().hex[:8]; changed = True
                        if not n.get("color"): n["color"] = NOTE_COLORS[i % len(NOTE_COLORS)]; changed = True
                                  n.setdefault("done", False)
    if changed: kv_save("notes", notes)
          return notes

@app.route("/api/notes", methods=["GET"])
def notes_list(): return jsonify({"notes": get_notes()})

@app.route("/api/notes", methods=["POST"])
def notes_add():
      data = request.get_json() or {}
    text = (data.get("text") or "").strip()
    if not text: return jsonify({"ok": False, "error": "Text required"}), 400
          notes = get_notes()
    ...
    notes.append(note); kv_save("notes", notes)
...
def notes_update(note_id):
    ...
    for n in notes:
              if n["id"] == note_id:
                            if "text" in data: n["text"] = (data["text"] or "").strip()
                                          if "color" in data and data["color"] in NOTE_COLORS: n["color"] = data["color"]
                                                        if "done" in data: n["done"] = bool(data["done"])
                                                                      break
else:
...
def quote():
      try:
                r = requests.get("https://zenquotes.io/api/today", timeout=6); r.raise_for_status()
                item = r.json()[0]
...
def weather_config():
      if request.method == "POST":
                ...
                if "lat" in data:
                              try: cfg["lat"] = float(data["lat"])
except (TypeError, ValueError): pass
        if "lon" in data:
                      try: cfg["lon"] = float(data["lon"])
except (TypeError, ValueError): pass
...
def weather():
      try:
                ...
                d = r.json(); cur = d.get("current", {}); daily = d.get("daily", {})
                return jsonify({"temp": cur.get("temperature_2m"),
                                "humidity": cur.get("relative_humidity_2m"),
                                "wind": cur.get("wind_speed_10m"),
                                "code": cur.get("weather_code"),
                                "description": weather_code_to_text(cur.get("weather_code")),
                                "high": (daily.get("temperature_2m_max") or [None])[0],
                                "low": (daily.get("temperature_2m_min") or [None])[0],
                                "location": cfg.get("name", DEFAULT_LOCATION_NAME)})
except Exception as e:
        return jsonify({"error": str(e)}), 500

def weather_code_to_text(code):
      mapping = {0:("Clear sky","☀️"),1:("Mainly clear","🌤️"),2:("Partly cloudy","⛅"),3:("Overcast","☁️"),
      ...
      text, emoji = mapping.get(code, ("Unknown", "?"))
      return f"{emoji} {text}"
...
def transport_config():
      if request.method == "POST":
                ...
                for key in ("from_stop", "to_stop"):
                              if key in data and data[key]: cfg[key] = data[key].strip()
                                        for key in ("walk_to_stop_min", "walk_from_stop_min"):
                                                      if key in data:
                                                                        try: cfg[key] = max(0, int(data[key]))
except (TypeError, ValueError): pass
...
def transport():
      try:
                ...
                if not cfg.get("from_stop") or not cfg.get("to_stop"):
                              return jsonify({"error": "Transport stops not configured", "from_stop": "", "to_stop": "",
                                                                          "walk_to_stop_min": cfg.get("walk_to_stop_min", 0),
                                                                          "walk_from_stop_min": cfg.get("walk_from_stop_min", 0),
                                                                          "connections": []})
                          url = f"http://transport.opendata.ch/v1/connections?from={cfg['from_stop']}&to={cfg['to_stop']}&limit=6"
        ...
        for c in connections:
                      dep = c.get("from", {}).get("departure"); arr = c.get("to", {}).get("arrival")
                      if not dep: continue
                                    dep_dt = parse_iso_datetime(dep); arr_dt = parse_iso_datetime(arr) if arr else None
            if dep_dt is None: continue
            ...
            if sections:
                          ...
                          results.append({"bus_line": line, "departure": dep_dt.strftime("%H:%M"),
                                                                      "arrival": arr_dt.strftime("%H:%M") if arr_dt else None,
                                                                      "departure_ts": dep_dt.timestamp(),
                                                                      "arrival_ts": arr_dt.timestamp() if arr_dt else None,
                                                                      "minutes_until_leave": round(mins_until_leave),
                                                                      "minutes_until_bus": round(mins_until_bus),
                                                                      "total_minutes": total_door_to_door})
        return jsonify({"from_stop": cfg["from_stop"], "to_stop": cfg["to_stop"],
                                                "walk_to_stop_min": walk_to, "walk_from_stop_min": walk_from,
                                                "connections": results})
except Exception as e:
        return jsonify({"error": str(e)}), 500
...
if __name__ == "__main__":
    ...
    print(f"Dashboard running at http://localhost:{port}  DB: {DB_URL}")
